...
router.post("/payment", async(req, res) => {
  ...
  const games=cartItems.map(item=>{
    return {title:item.split('%')[0],subtotal_price:parseInt(item.split('%')[1])}
  })
...
router.get('/payment', async(req,res,next)=>{
  ...
  try{
    ...
    const {data}=await axios.post('http://localhost:3001/order/postOrder',order)
    if(!data)return res.status(400).send('Order not created')
    
    if(req.query.status==='approved'){
      //actualizar juegos comprados
      await axios.put(`http://localhost:3001/users/userGames/${order.user_id}`,{cartItems:games_id})
    }
